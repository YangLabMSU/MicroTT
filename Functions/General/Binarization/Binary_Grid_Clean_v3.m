function [BWContourImage2] = Binary_Grid_Clean_v3(BWImage, ContourSizeThresh, FillStyle)
...
% Check to see if there are two contours that could be nearly the same size
if size(ContourBoundaries2,1) > 1

    MaxContSize = size(ContourBoundaries2{end}, 1) ;
    SecContSize = size(ContourBoundaries2{end-1}, 1) ;
    SizediffPer = abs(MaxContSize-SecContSize) / MaxContSize ;

    if SizediffPer < 0.3
        MaxContour = [ ContourBoundaries2{end} ; ContourBoundaries2{end-1} ] ;
    else
