function [Grids] = Auto_SetWBPixelRatio_v4(Grids, AutoTemplate)
...
Grids.TopStructure.InitialBW = HM_Binarize_Threshold_Adjust_v2(TGI, TGTBW, 0.1) ;

% Clean Initial Grid - Top
% Grids.TopStructure.InitialBW = Binary_Grid_Clean(Grids.TopStructure.InitialBW) ;
Grids.TopStructure.InitialBW = Binary_Grid_Clean_v3(Grids.TopStructure.InitialBW, 200, "") ; 
...
Grids.BottomStructure.InitialBW = HM_Binarize_Threshold_Adjust_v2(BGI, BGTBW, 0.1) ;

% Clean Initial Grid - Bottom
% Grids.BottomStructure.InitialBW = Binary_Grid_Clean(Grids.BottomStructure.InitialBW) ;
Grids.BottomStructure.InitialBW = Binary_Grid_Clean_v3(Grids.BottomStructure.InitialBW, 200, "") ;
...
Grids.Fiber.InitialBW  = Binary_Grid_Clean_v3(Grids.Fiber.InitialBW , 200, "") ;
% 
...
TBPRDiff = abs(TRGIBPR-TLGIBPR) ;
if TBPRDiff > 0.1 || TLGIBPR == 0 || TRGIBPR == 0
    PixelCompVal = 0.2 ;
    Iterations = 0 ;
    while TBPRDiff > 0.05 && Iterations < 40 || TLGIBPR == 0 || TRGIBPR == 0

        %Grids.TopStructure.InitialBW = HM_Binarize_Threshold_Adjust(TGI, TGTBW) ;
        PixelCompVal = PixelCompVal - 0.01 ;

        Grids.TopStructure.InitialBW = HM_Binarize_Threshold_Adjust_v2(TGI, TGTBW, PixelCompVal) ;
        ...
        TBPRDiffTracker(Iterations) = TBPRDiff ;

    end

end

BBPRDiff = abs(BRGIBPR-BLGIBPR) ;
if BBPRDiff > 0.05 || BLGIBPR == 0 || BRGIBPR == 0
    PixelCompVal = 0.2 ;
    Iterations = 0 ;
    while BBPRDiff > 0.05 && Iterations < 40 || BLGIBPR == 0 || BRGIBPR == 0

        %Grids.TopStructure.InitialBW = HM_Binarize_Threshold_Adjust(TGI, TGTBW) ;
        PixelCompVal = PixelCompVal - 0.01 ;

        Grids.BottomStructure.InitialBW = HM_Binarize_Threshold_Adjust_v2(BGI, BGTBW, PixelCompVal) ;
        ...
        BBPRDiff = abs(BRGIBPR-BLGIBPR) ;
        Iterations = Iterations + 1 ;
        BBPRDiffTracker(Iterations) = BBPRDiff ;
