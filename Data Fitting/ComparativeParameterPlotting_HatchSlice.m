function ComparativeParameterPlotting_HatchSlice(ComparisonData)
...
for n = 1:size(sort_index,1) 
    ...
    LegendString(n,1) = sprintf("Hat x Slice: %s, SEM Area: %s", string(Variables(n,1)), string(SEMArea(n,1))) ; 
...
title(leg, 'Hatch x Slice (um), Area (um^2)') ;
ylim([0 1.75]) ;
