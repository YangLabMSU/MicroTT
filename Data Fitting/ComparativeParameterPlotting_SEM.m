function ComparativeParameterPlotting_SEM(ComparisonData)

SEMArea = ComparisonData{1,7} ;
[SEMArea, sort_index] = sort(SEMArea);

Variables = ComparisonData{1,6} ;
Variables = Variables(sort_index,1) ;

for n = 1:size(sort_index,1)

    LegendString(n,1) = sprintf("%s, SEM Area: %s", string(Variables(n,1)), string(SEMArea(n,1))) ;
...
hold off;
subplot(3,2,[1 2]) ; 
b3 = bar(Modulus', 'FaceColor', 'flat', 'LineWidth', 1); hold on ; 
ylim([0 20000]) ; 
leg = legend(b3, LegendString, 'Location', 'north', 'FontSize', 12, 'box', 'off') ;
title(leg, 'Parameters, Area (um^2)') ;
set(b3, 'Visible', 'off');
axis off ; 
