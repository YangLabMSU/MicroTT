...
VariablesNew = [ "Perp.", "Angled", "Parallel"]' ; 
i = 1 ;
for n = 1:size(HatComps, 1)
    ...
    if sum(matchingRows) > 1
        ...
        for nn = 1:size(CompData,1)
            ...
            LegendString(nn,1) = string(VariablesNew(nn,1)) ; % + ", Area: " + string(round(double(SEMArea),2)) + ' \mum^2' ;

        end

        leg = legend(plothandles, LegendString,...
            'Location', 'northwest', 'FontSize', 15, 'box', 'on', 'LineWidth', 4, 'edgecolor', 'none') ; , ...
            %'box', 'off') ;

        title(leg, 'Hatch Direction') ;

        ylim([0 90]) ;
        yticks(linspace(0,90,7)) ;

        xlim([-0.02 0.9]) ;
        xticks(round(linspace(0,0.9,4),2)) ;
        ...
        % Plot Save
        path = "K:\Yang Research\Two-Photon Polymerization\Analysis Code\DIC v12 - 4-26-24\Data Fitting\Ogden Fitting Plotting\Hatch Direction" ;
        fileName = n + "_SS-Plot-v2";
