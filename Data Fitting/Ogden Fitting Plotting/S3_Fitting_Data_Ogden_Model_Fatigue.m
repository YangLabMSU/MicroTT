...
New_All_IPS_Data = StretchExperimentData(:, [1:4 7]) ; 
...
if MaxStrain > 0.15
    ...
    StrainIdx1 = find(Strain > 0.15, 1, "first") ;
    StrainInterp1 = linspace(Strain(1), Strain(StrainIdx1), 250) ;

    DownStrain = MaxStrain - 0.15 ;
    NumIntPts = round(DownStrain*200) ; 
...
PlottingSummaryFunction_Ogden_Engineering(Stress, Strain, Lambda, Parameters, CriticalPts, FittingValues, GoodnessOfFit) ; 
%PlottingSummaryFunction_Ogden_True(TrueStress, TrueStrain, Lambda, Parameters, CriticalPts, FittingValues, GoodnessOfFit) ; 
...
save("All_IPS_LargeStrain_Data_TrueOgden_Fatigue.mat", "All_IPS_Data_Ogden") ; %, "All_IPS_Data_Table") ; 
