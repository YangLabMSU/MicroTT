...
for n = 1 %:size(CompDataTable)
    ...
    NE = [] ; 
    for n = 1:size(YStrainCell,2)
        NumExps = size(YStrainCell{n},1) ; 
        NE = [ NE ; NumExps ] ; 
    end
    disp(NE) ; 
    ...
    text(7.5, 55, 'Yield Stress, \sigma_y', 'FontSize', FS, 'FontWeight', 'bold', 'HorizontalAlignment', 'center') ;
    ax = gca;
    ...
    text(9, 1300, '\mu_1', 'FontSize', FS, 'FontWeight', 'bold', 'HorizontalAlignment', 'center') ;
    ax = gca;
    ax.Box = 'on';  % Turn on the box
    set(ax, 'BoxStyle', 'full');  % Ensure the box is drawn on all sides
    ax.LineWidth = 1.25 ; % Increase the axis line width
    ...
    text(2, -150, '\mu_2', 'FontSize', FS, 'FontWeight', 'bold', 'HorizontalAlignment', 'center') ;
    ax = gca;
    ax.Box = 'on';  % Turn on the box
    set(ax, 'BoxStyle', 'full');  % Ensure the box is drawn on all sides
    ax.LineWidth = 1.25 ; % Increase the axis line width
    ...
    text(2, -17, '\alpha_1', 'FontSize', FS, 'FontWeight', 'bold', 'HorizontalAlignment', 'center') ;
    ax = gca;
    ax.Box = 'on';  % Turn on the box
    set(ax, 'BoxStyle', 'full');  % Ensure the box is drawn on all sides
    ax.LineWidth = 1.25 ; % Increase the axis line width
    ...
    text(9, 12.5, '\alpha_2', 'FontSize', FS, 'FontWeight', 'bold', 'HorizontalAlignment', 'center') ;
    ax = gca;
    ax.Box = 'on';  % Turn on the box
    set(ax, 'BoxStyle', 'full');  % Ensure the box is drawn on all sides
    ax.LineWidth = 1.25 ; % Increase the axis line width
    ...
    % disp(Details2);
    % 
    % fileName1 = n+"_YS"+'_Fig1_v2'+'.tiff';
    % fileName2 = n+"_mu1"+'_Fig2_v2'+'.tiff';
    % fileName3 = n+"_mu2"+'_Fig3_v2'+'.tiff';
    % fileName4 = n+"_alpha1"+'_Fig4_v2'+'.tiff';
    % fileName5 = n+"_alpha2"+'_Fig5_v2'+'.tiff';
    % path = cd ;
