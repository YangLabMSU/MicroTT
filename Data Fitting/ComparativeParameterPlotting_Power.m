function ComparativeParameterPlotting_Power(ComparisonData)
...
for n = 1:size(sort_index,1) 
    ...
    LegendString(n,1) = sprintf("Power: %s. SEM Area: %s", string(Variables(n,1)), string(SEMArea(n,1))) ; 
...
% leg = legend(Variables, 'Location', 'north', 'FontSize', 12, 'box', 'off') ;
% title(leg, 'Power (%max)') ;
...
% Bar chart with error bars
%b2 = bar(x2, Lamda, 'FaceColor', 'flat');
