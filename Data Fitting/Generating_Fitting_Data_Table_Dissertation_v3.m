...
for n = 1:size(IPS_Exps_Idx,1)
    ...
    FitValues = AveragedStretchExps{Idx, 7} ;
    YieldValues = AveragedStretchExps{Idx, 8} ;
...
%sortOrder = 2 ; 
%sortIdx = sortRows(double(TableVector(:,2))) ; 
[~, sort_index] = sort(double(TableVector(:,2)));
% Sort the matrix using the specified columns in the given order
%sortedMatrix = sortrows(TableVector, sortOrder);
sortedMatrix = TableVector(flip(sort_index),:) ; 
...
% save("Averaged Stretch Experiments Data Table - Sub 6-14", "AveragedStretchExpsDataTable") ; 
