function customBoxPlot_withStats_v9(dataCell)
...
for n = 1:numCategories
    ...
    if ~isempty(Data)
        ...
        if boxBottom > minValue && boxTop < maxValue
            plot([n, n], [minValue, boxBottom], 'k--', 'LineWidth', LW); hold on;
            plot([n, n], [boxTop, maxValue], 'k--', 'LineWidth', LW);
            plot([n - (boxwidth / 4), n + (boxwidth / 4)], [maxValue, maxValue], 'k-', 'LineWidth', LW);
            plot([n - (boxwidth / 4), n + (boxwidth / 4)], [minValue, minValue], 'k-', 'LineWidth', LW);
        end

        fill([n - (boxwidth / 2), n + (boxwidth / 2), n + (boxwidth / 2), n - (boxwidth / 2)], [boxBottom, boxBottom, boxTop, boxTop], 'b', 'FaceAlpha', 0.1, 'LineWidth', LW); hold on;
...
for i = 1:numCategories
    ...
    for j = i+1:numCategories
            ...
            if p < 0.05  && SigCount < 2 %&& comparisonCount < 6
                xPos = (i + j) / 2;
                ...
                % Draw the bracket
                line([i, i, j, j], [yPos - yOffset/4, yPos, yPos, yPos - yOffset/4], 'Color', 'k', 'linewidth', LW);
                SigCount = SigCount + 1 ;
            end
    end
end
