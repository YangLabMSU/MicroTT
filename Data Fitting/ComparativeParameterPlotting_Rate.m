function ComparativeParameterPlotting_Rate(ComparisonData)
...
for n = 1:size(sort_index,1) 
    ...
    LegendString(n,1) = sprintf("Strain Rate: %s, SEM Area: %s", string(Variables(n,1)), string(SEMArea(n,1))) ; 
...
title(leg, 'Strain Rate (um/s), Area (um^2)') ;
ylim([0 1.75]) ;
