function [FittingValues, GoodnessOfFit] = TPPTT_MidStrainFittingFunc_v2(InitialGuess, FitParameters, Stress, Strain) 
...
%problem.objective = @(x)PolymerEquations_2Term(x, FitParameters, Stress, Strain) ;
problem.objective = @(x)PolymerEquations3_v2(x, PV, Stress, Strain) ; 
problem.x0 = x0 ;
...
StressFit = x(1).*StrainFit - x(2)*exp(-(x(3)./StrainFit)) ; 
