function ComparativeParameterPlotting_HatchStyle(ComparisonData)
...
for n = 1:size(sort_index,1) 
    ...
    LegendString(n,1) = sprintf("Hatch Stlye: %s, SEM Area: %s", string(Variables(n,1)), string(SEMArea(n,1))) ; 
...
title(leg, 'Hatch Style, Area (um^2)') ;
ylim([0 1.75]) ;
