...
for n = 1:size(HSComps, 1)
    ...
    if sum(matchingRows) > 1
        ...
        Variables = double(Data(matchingRows, HSCompCol)) ;
        [~, sort_index] = sort(Variables(:,2));
        ...
        Details = sprintf("%s, Power: %s mW, Speed: %s mm/s, w x h: %s x %s um, Style: %s, Strain Rate: %s um/s", ...
            ConstantParams(1,:)) ;
        ...
        for nn = 1:size(CompData,1)
            ...
            LegendString(nn,1) = string(sprintf('Hat x Slice: %s um, SEM Area: %s um^2', string(Variables(nn,1)), string(SEMArea(nn,1)))) ;

        end

        leg = legend(plothandles, LegendString,...
            'Location', 'northwest', 'FontSize', 15, 'box', 'off', 'LineWidth', 5) ;
        title(leg, 'Hatch x Slice Comparison') ;
        ...
        newFolder = 'Hatch x Slice Comparisons - Averaged Curves' ;
