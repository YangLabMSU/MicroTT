...
for n = 1:size(HatComps, 1)
    ...
    if sum(matchingRows) > 1
        ...
        for nn = 1:size(CompData,1)
            ...
            LegendString(nn,1) = string(sprintf('Hatch: %s, Slice: %s, SEM Area: %s um^2', string(Variables(nn,1)), string(Variables(nn,2)), string(round(double(SEMArea(nn,1)),2)))) ;
        ...
        newFolder = 'Hatch Slice Comparisons - Averaged Curves - 6-5-24' ;
