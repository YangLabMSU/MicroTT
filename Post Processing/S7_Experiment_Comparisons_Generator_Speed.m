...
for n = 1:size(SpdComps, 1)
    ...
    if sum(matchingRows) > 1
        ...
        for nn = 1:size(CompData,1)
            ...
            LegendString(nn,1) = string(sprintf('Speed: %s mm/s, SEM Area: %s um^2', string(Variables(nn,1)), string(SEMArea(nn,1)))) ;
        ...
        newFolder = 'Write Speed Comparisons - Averaged Curves' ;
