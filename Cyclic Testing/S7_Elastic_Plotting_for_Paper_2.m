...
for i = 0:length(TrIdx)-1
    if i == 0
        ...
        segmentTime = ElasticTime(1:TrIdx(1));        
        segmentFiberStrain = ElasticFiberStrain(1:TrIdx(1));
        segmentPlateStrain = ElasticPlateStrain(1:TrIdx(1)); 
        ...
        segmentTime = ElasticTime(TrIdx(i):TrIdx(i+1)-1);
        segmentFiberStrain = ElasticFiberStrain(TrIdx(i):TrIdx(i+1)-1);
        segmentPlateStrain = ElasticPlateStrain(TrIdx(i):TrIdx(i+1)-1); 
...
xlabel("Time (s)");
ylim([-0.002 0.015]) ; 
yticks(linspace(0,0.015, 4)) ; % [0 0.005 0.01 0.015]) ;
