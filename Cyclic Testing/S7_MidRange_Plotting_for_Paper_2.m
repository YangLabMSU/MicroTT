clear ; clc ; close all ;
% Plotting Elastic Experiment for Paper

DataPath = "K:\Yang Research\Two-Photon Polymerization\Tensile Testing Data\Second Paper\Cyclic Data - v3 - Analyzed\Substrate 9-05-23_TD 10-18-23\16" ; % \Processed Data.mat" ;
Data = load(fullfile(DataPath, "Processed Data.mat"));
Fit = load(fullfile(DataPath, "Fit Data.mat"));
SavePath = "K:\Advancing TPP Paper - v2\Figure Folder\Figure 6 - Cyclic Testing" ;

% Extract necessary data
FitTimes = Fit.FitData.Stress.Trtime;
Time = Data.ProcessedData.Time;
ElasticStress = Data.ProcessedData.Stress-Data.ProcessedData.Stress(1);
PlateStrain = Data.ProcessedData.PlateStrain-Data.ProcessedData.PlateStrain(1);
FiberStrain = Data.ProcessedData.FiberStrain-Data.ProcessedData.FiberStrain(1);

% Find indices for the troughs
TrIdx = zeros(size(FitTimes, 1), 1);
for n = 1:size(FitTimes, 1)
    TrIdx(n,1) = find(Time >= FitTimes(n), 1, "first");
end

% Add the last index for the full range
TrIdx = [TrIdx; length(Time)];

% Set up figure
% MedPlotSize = [10, 5, 4.1*4, 1.75*4];
MedPlotSize = [10, 5, 7.1*3, 4.1*3];
fig1 = figure('units', 'centimeters', 'position', MedPlotSize);
hold on;

plot(Time(1:end-50), PlateStrain(1:end-50), 'Color', "#bfbfc7", 'LineWidth', 4) ; 

% Set colormap for gradient
cmap = turbo(length(TrIdx) - 1); % Adjust the number of colors to match cycles
colormap(cmap); % Apply colormap

% Plot each segment with a unique color
for i = 0:length(TrIdx)-1
    if i == 0
        % Segment data for the first cycle
        segmentTime = Time(1:TrIdx(1));
        segmentFiberStrain = FiberStrain(1:TrIdx(1));

        % Plot for Fiber Strain
        plot(segmentTime, smooth(segmentFiberStrain), 'Color', cmap(1, :), 'LineWidth', 4, ...
            'DisplayName', sprintf('Cycle %d (Fiber)', i)); hold on ;
    else
        % Segment data for each subsequent cycle
        segmentTime = Time(TrIdx(i):TrIdx(i+1)-1);
        segmentFiberStrain = FiberStrain(TrIdx(i):TrIdx(i+1)-1);

        % Plot for Fiber Strain
        plot(segmentTime, smooth(segmentFiberStrain), 'Color', cmap(i, :), 'LineWidth', 4, ...
            'DisplayName', sprintf('Cycle %d (Fiber)', i));
    end
end

% Customize plot
ylabel("Strain");
xlabel("Time (s)");
ylim([-0.055 0.4]) ;
yticks(linspace(0,0.4,5)) ; % [0 0.005 0.01 0.015]) ;

set(gca, 'FontSize', 20);
ax = gca;
% ax.YAxis.Exponent = 0; % Disable scientific notation on the x-axis
% ax.YTickLabel = arrayfun(@(x) sprintf('%.3f', x), ax.YTick, 'UniformOutput', false); % Set tick labels

ax.Box = 'on';  % Turn on the box
set(ax, 'BoxStyle', 'full');  % Ensure the box is drawn on all sides
ax.LineWidth = 1.5; % Increase the axis line width
ax.TickLength = [0.01 0.01]; % Increase the length of the tick marks
grid on;

% Add colorbar and adjust properties
% cbar = colorbar; % Get the colorbar handle
% cbar.Ticks = linspace(0, 1, 5); % Set ticks based on the number of cycles
% cbar.TickLabels = linspace(0, length(TrIdx), 5) ; % Label the ticks from 0 to the number of cycles (60)
% cbar.Label.String = 'Strain Cycle'; % Add label to the colorbar

% Save the figure
fileName6 = 'MidRange_Strain_v3';
filename6 = fullfile(SavePath, fileName6);  % Set your desired file name
print(filename6, '-djpeg', '-r300');  % Save as JPEG with 300 dpi resolution
