...
% y(t) = A * exp(-lambda * t) * sin(omega * t + phi) + C0 * exp(C1 * t) + C_init
decaying_sinusoidal_model_with_exp_offset = @(b, x) ...
    b(1) * exp(-b(2) * x) .* sin(b(3) * x + b(4)) + b(5) * exp(b(6) * x) + b(7);

% Initial guesses for parameters [A, lambda, omega, phi, C0, C1, C_init]
initial_guess_stress = [amplitude_stress, 0.01, 2*pi/average_period, 0, mean(stress), 0.01, 0.4];
initial_guess_strain = [amplitude_strain, 0.01, 2*pi/average_period, 0, mean(strain), 0.01, 0.7];

% Fit the model to the stress data
fit_params_stress = lsqcurvefit(decaying_sinusoidal_model_with_exp_offset, initial_guess_stress, trimmed_time, trimmed_stress);

% Fit the model to the strain data
fit_params_strain = lsqcurvefit(decaying_sinusoidal_model_with_exp_offset, initial_guess_strain, trimmed_time, trimmed_strain);

% Generate fitted data
fitted_stress = decaying_sinusoidal_model_with_exp_offset(fit_params_stress, trimmed_time);
fitted_strain = decaying_sinusoidal_model_with_exp_offset(fit_params_strain, trimmed_time);
...
xlabel('Time'); ylabel('Stress'); title('Stress vs Time with Decaying Sinusoidal and Exponential Offset');
legend show;
...
xlabel('Time'); ylabel('Strain'); title('Strain vs Time with Decaying Sinusoidal and Exponential Offset');
legend show;

% Plot the decay term and exponential offset term
decay_term_stress = fit_params_stress(1) * exp(-fit_params_stress(2) * trimmed_time);
exp_offset_term_stress = fit_params_stress(5) * exp(fit_params_stress(6) * trimmed_time) + fit_params_stress(7);
decay_term_strain = fit_params_strain(1) * exp(-fit_params_strain(2) * trimmed_time);
exp_offset_term_strain = fit_params_strain(5) * exp(fit_params_strain(6) * trimmed_time) + fit_params_strain(7);

subplot(3,1,3);
plot(trimmed_time, decay_term_stress, 'g', 'DisplayName', 'Decay Term (Stress)');
hold on;
plot(trimmed_time, exp_offset_term_stress, 'b', 'DisplayName', 'Exponential Offset Term (Stress)');
plot(trimmed_time, decay_term_strain, 'm', 'DisplayName', 'Decay Term (Strain)');
plot(trimmed_time, exp_offset_term_strain, 'r', 'DisplayName', 'Exponential Offset Term (Strain)');
xlabel('Time'); ylabel('Amplitude'); title('Decay and Exponential Offset Terms over Time');
legend show;
