...
% y(t) = A * exp(-lambda * t) * sin(omega * t + phi) + C0 * exp(C1 * t)
decaying_sinusoidal_model_with_exp_offset = @(b, x) ...
    b(1) * exp(-b(2) * x) .* sin(b(3) * x + b(4)) + b(5) * exp(b(6) * x);

% Initial guesses for parameters [A, lambda, omega, phi, C0, C1]
initial_guess_stress = [amplitude_stress, 0.01, 2*pi/average_period, 0, mean(stress), 0.01];
initial_guess_strain = [amplitude_strain, 0.01, 2*pi/average_period, 0, mean(strain), 0.01];
...
exp_offset_term_stress = fit_params_stress(5) * exp(fit_params_stress(6) * trimmed_time);
decay_term_strain = fit_params_strain(1) * exp(-fit_params_strain(2) * trimmed_time);
exp_offset_term_strain = fit_params_strain(5) * exp(fit_params_strain(6) * trimmed_time);
