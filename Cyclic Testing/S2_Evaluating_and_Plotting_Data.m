...
for n = 2 %1:size(DataDirectory,1)
    ...
    % Fit an exponential decay to the peaks
    % Assuming peak amplitudes decrease according to A(t) = A0 * exp(-lambda * t)

    % Use only the peaks of the stress data for this example
    peak_amplitudes = pks_stress;
    peak_times = locs_stress;

    % Fit an exponential decay function to the peak amplitudes
    exp_decay_model = @(b, x) b(1) * exp(-b(2) * x);

    % Initial guess for parameters [A0, lambda]
    initial_guess_exp = [peak_amplitudes(1), 0.01];

    % Fit the model to the peak data
    fit_params_exp = lsqcurvefit(exp_decay_model, initial_guess_exp, peak_times, peak_amplitudes);

    % Extract the decay constant lambda
    decay_constant_lambda = fit_params_exp(2);

    % Define the decaying sinusoidal model function
    decaying_sinusoidal_model = @(b, x) b(1) * sin(b(2) * x + b(3)) .* exp(-decay_constant_lambda * x) + b(4);

    % Initial guess for parameters [amplitude, angular frequency, phase, offset]
    initial_guess_stress = [amplitude_stress, 2*pi/average_period, 0, mean(trimmed_stress)];
    initial_guess_strain = [amplitude_strain, 2*pi/average_period, 0, mean(trimmed_strain)];

    % Fit the decaying sinusoidal model to the trimmed stress data
    fit_params_stress = lsqcurvefit(decaying_sinusoidal_model, initial_guess_stress, trimmed_time, trimmed_stress);

    % Fit the decaying sinusoidal model to the trimmed strain data
    ...
    xlabel('Time');
    ylabel('Stress');
    title('Trimmed Stress vs Time with Decaying Sinusoidal Fit');
    legend show;
    ...
    xlabel('Time');
    ylabel('Strain');
    title('Trimmed Strain vs Time with Decaying Sinusoidal Fit');
    legend show;
