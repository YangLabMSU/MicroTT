...
for n = 3 %1:size(DataDirectory)
    ...
    stress = ProcessedData.Stress(1:4600);
    fiberstrain = ProcessedData.FiberStrain(1:4600);
    platestrain = ProcessedData.PlateStrain(1:4600);
    time = ProcessedData.Time(1:4600);
    ...
    ylim([-0.05 0.55]) ;
    yLimits = ylim ;
    yticks(round(linspace(0, 0.4, 5),3)) ;
    xlim([0 max(time)*1.05])
    xLimits = xlim ;
    xticks(round(linspace(0,xLimits(2)-1,6))) ;
    ...
    xlim([-0.05 0.4])
    xLimits = xlim ;
    xticks(round(linspace(0, xLimits(2), 5),3)) ;
