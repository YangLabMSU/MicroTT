...
for n = 1 %1:size(DataDirectory)
    ...
    ylim([0 140])
    yLimits = ylim ;
    yticks(round(linspace(0, yLimits(2), 5))) ;
    ...
    leg = legend('Plate Strain', 'Fiber Strain', 'location', 'north', 'box', 'off', 'fontsize',9);
    leg.NumColumns = 2 ;
    ax = gca;
    ax.Box = 'on';  % Turn on the box
    set(ax, 'BoxStyle', 'full');  % Ensure the box is drawn on all sides
    ...
    ylim([-0.2 1]) ;
    yLimits = ylim ;
    yticks(round(linspace(-0.2, 0.8, 6),3)) ;
    xlim([0 max(time)*1.05])
    xLimits = xlim ;
    xticks(round(linspace(0,xLimits(2)-1,6))) ;
    ...
    ylim([0 140]) ; 
    yLimits = ylim ;
    yticks(round(linspace(0,yLimits(2),6))) ;
    xlim([-0.2 0.8])
    xLimits = xlim ;
    xticks(round(linspace(xLimits(1), xLimits(2), 6),3)) ;
    ...
    ylim([120 128]) ; 
    yLimits = ylim ;
    yticks(round(linspace(yLimits(1), yLimits(2), 5),1)) ;
    ...
    ylim([0.62 0.7])
    yLimits = ylim ;
    yticks(round(linspace(yLimits(1), yLimits(2), 5),4)) ;
    ...
    yLimits = ylim ;
    % yLimits = ylim ;
    yticks(round(linspace(yLimits(1), yLimits(2), 5),5)) ;
    leg = legend("Peaks", "Troughs", 'Location', 'south', 'fontsize', 8, 'box', 'off') ;
    leg.NumColumns = 2; 
