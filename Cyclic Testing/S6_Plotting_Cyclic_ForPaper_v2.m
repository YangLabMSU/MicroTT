...
LargePlotSize1 = [ 10, 5, 10*3, 2.25*3 ] ;
LargePlotSize2 = [ 10, 5, 10*3, 1.75*3 ] ;
MedPlotSize = [10, 5, 5*3, 3.5*3] ; 

fig1 = figure('units', 'centimeters', 'position', LargePlotSize1); hold on;
...
grid on ;
xlim([0 1250]) ;
xticks(linspace(0,1250,6)) ;
ylim([-.2 0.8]) ;
yticks(linspace(-0.2, 0.8, 6)) ;

fileName1 = 'LargeRange_Strain_v2';
...
fig7 = figure('units', 'centimeters', 'position', LargePlotSize1); hold on;
plot(DRTime, smooth(DRStress), 'linewidth', 5, 'DisplayName', "Stress"); hold on;
ylabel("Stress (MPa)");
xlabel("Time (s)") ;
set(gca, 'FontSize', 20) ;
ax = gca;
...
xticks(linspace(0,1250,6)) ;
ylim([0 150]) ;
yticks(linspace(0, 150, 4)) ;

fileName7 = 'LargeRange_Stress_v2';
filename7 = fullfile(path,fileName7);  % Set your desired file name
print(filename7, '-djpeg', '-r300');  % Save as JPEG with 300 dpi resolution
