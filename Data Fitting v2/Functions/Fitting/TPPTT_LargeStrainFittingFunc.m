function [FittingValues, GoodnessOfFit] = TPPTT_LargeStrainFittingFunc(InitialGuess, FitParameters, Stress, Strain) 
...
StrainFit = Strain ; %linspace(0, Strain(end), 100) ; 
StressFit = E1.*StrainFit - x(1)*exp(-(x(2)./StrainFit)) - x(3)*exp(-(x(4)./StrainFit)) ; 
